...
Hooks:Add("MenuManagerInitialize", "fray_initmenu", function(menu_manager)
	local show_popup = nil
	MenuCallbackHandler.callback_fray_toggle_overhaul_player = function(self,item) --toggle
		PD2FRAY:ChangeSetting("toggle_overhaul_player",item:value() == "on")
		PD2FRAY.show_popup = true
	end
	
	MenuCallbackHandler.callback_fray_toggle_hhskulldiff = function(self,item) --toggle
		PD2FRAY:ChangeSetting("toggle_hhskulldiff",item:value() == "on")
	end
	
	MenuCallbackHandler.callback_fray_toggle_health_effect = function(self,item) --toggle
		PD2FRAY:ChangeSetting("toggle_health_effect",item:value() == "on")
	end
	
	MenuCallbackHandler.callback_fray_toggle_noweirddof = function(self,item) --toggle
		PD2FRAY:ChangeSetting("toggle_noweirddof",item:value() == "on")
	end
	
	MenuCallbackHandler.callback_fray_toggle_blurzonereduction = function(self,item) --toggle
		PD2FRAY:ChangeSetting("toggle_blurzonereduction",item:value() == "on")
	end
	
	MenuCallbackHandler.callback_fray_toggle_suppression = function(self,item) --toggle
	...
	MenuCallbackHandler.callback_fray_close = function(self,item) --toggle
		if PD2FRAY and PD2FRAY.show_popup then 
			local has_change = false
			local settings_list = "\n"
			for setting_name,value in pairs(PD2FRAY.session_settings) do 
				local new_value = PD2FRAY.settings[setting_name]
				if (new_value ~= nil) and (new_value ~= value) then 
					has_change = true
					settings_list = settings_list .. managers.localization:text("fray_" .. tostring(setting_name) .. "_title") .. ": " .. tostring(value) .. "-->" .. tostring(new_value) .. "\n"
				end
			end
			if has_change then 
				QuickMenu:new(managers.localization:text("fray_requires_restart_title"),string.gsub(managers.localization:text("fray_requires_restart_desc"),"$SETTINGSLIST",settings_list),{
					{
	...
	MenuHelper:LoadFromJsonFile(PD2FRAY._options_path, PD2FRAY, PD2FRAY.settings)
end)
...
Hooks:Add("MenuManagerBuildCustomMenus", "FRAY_CONTROLS", function(menu_manager, nodes)
	local controls_node = nodes.controls

	local params = {
		name = "hold_to_jump",
		text_id = "fraymenu_hold_to_jump",
		help_id = "fraymenu_hold_to_jump_help",
	...
	local params = {
		name = "staticrecoil",
		text_id = "fraymenu_staticrecoil",
		help_id = "fraymenu_staticrecoil_help",
	...
	local params = {
		name = "holdtofire",
		text_id = "fraymenu_holdtofire",
		help_id = "fraymenu_holdtofire_help",
...
Hooks:PostHook(MenuOptionInitiator, "modify_controls", "FRAY_modify_controls", function(self, node)
