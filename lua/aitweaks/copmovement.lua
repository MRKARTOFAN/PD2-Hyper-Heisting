...
function CopMovement:sync_action_act_start(index, blocks_hurt, clamp_to_graph, needs_full_blend, start_yaw, start_pos)
	if self._ext_damage:dead() then
		return
	end
	...
	local actual_start_rot = nil
	if type(start_yaw) == "number" and start_yaw ~= 0 then
		actual_start_rot = Rotation(start_yaw * 360 / 255, 0, 0)
	elseif type(start_yaw) == "userdata" and start_yaw.yaw then
		actual_start_rot = start_yaw
	end
...
-- ==========================================
-- Ultimate Action Block Crash Fix
-- ==========================================
local orig_CopMovement_chk_action_forbidden = CopMovement.chk_action_forbidden
function CopMovement:chk_action_forbidden(action_type)
	-- Before the engine checks the actions, we scan them ourselves
	if self._active_actions then
		for _, action in pairs(self._active_actions) do
			-- If an active action is missing its blocks table (because of a mod's early return),
			-- we instantly inject an empty table on the fly.
			if action and type(action) == "table" and action.chk_action_forbidden and not action._blocks then
				action._blocks = {}
			end
		end
	end
