...
if CopBrain._logic_variants.marshal_marksman then
...
Hooks:PostHook(CopBrain, "on_suppressed", "hh_on_suppressed_voiceline", function(self, state)
	...
	elseif self._logic_data.char_tweak.chatter and self._logic_data.char_tweak.chatter.suppress then
...
function CopBrain:update(unit, t, ...)
	if self._next_logic_upd_t <= t then
		...
		return _update_original(self, unit, t, ...)
	end
end
