...
Hooks:PostHook(GroupAIStateBase, "on_enemy_unregistered", "hh_on_enemy_unregistered_sh", function(self, unit)
	...
	if not e_data or not e_data.group or not e_data.group.has_spawned or not e_data.spawn_group then return end
	local max_dis = tweak_data.group_ai.spawn_kill_max_dis
	if not max_dis then return end
	local dis = mvector3.distance(e_data.spawn_group.pos, e_data.m_pos)
	if dis > max_dis then return end
...
function GroupAIStateBase:is_nav_seg_area_safe(skip_areas, nav_seg)
	for _, area in pairs(skip_areas) do
		if area.nav_segs[nav_seg] then return true end
	end
...
Hooks:PreHook(GroupAIStateBase, "add_special_objective", "hh_add_special_objective_sh", function(self, id, objective_data)
	...
	if objective_data.objective then
