...
function PlayerStandard:_update_hh_action_timers(t, dt, stop_running_in_air)
	...
	if stop_running_in_air and self._state_data.in_air_enter_t and t - self._state_data.in_air_enter_t > 0.1 then
		self:_interupt_action_running(t)
	end
end
...
local function HHUpgradeValueAny(category, upgrade, fallback_upgrade, second_fallback_upgrade, default)
	...
	if second_fallback_upgrade and managers.player:has_category_upgrade(category, second_fallback_upgrade) then
		return managers.player:upgrade_value(category, second_fallback_upgrade, default)
	end

	return default
end

local function FRAYHasUpgrade(category, upgrade, fallback_upgrade)
	return managers.player:has_category_upgrade(category, upgrade) or fallback_upgrade and managers.player:has_category_upgrade(category, fallback_upgrade)
end
...
function PlayerStandard:_do_action_melee(t, input, skip_damage)
	...
	if managers.player:has_category_upgrade("player", "beatemup_basic") or FRAYHasUpgrade("player", "acupuncture_basic", "draw_of_the_sword_basic") then
...
function PlayerStandard:_do_melee_damage(t, bayonet_melee, melee_hit_ray, melee_entry, hand_id)
	...
	if col_ray and alive(col_ray.unit) then
		...
		if character_unit:character_damage() and character_unit:character_damage().damage_melee then
			...
			dmg_multiplier = dmg_multiplier * managers.player:temporary_upgrade_value("temporary", "surge_of_power_revive_melee_damage_multiplier", 1)
			dmg_multiplier = dmg_multiplier * (1 + managers.player:upgrade_value("player", "yield_my_flesh_melee_damage_taken_multiplier", 0) * managers.player:yield_my_flesh_damage_ratio())
			...
			if managers.player:has_category_upgrade("melee", "stacking_hit_damage_multiplier") then
				...
				if defense_data and defense_data ~= "friendly_fire" and alive(character_unit) and not character_unit:character_damage():dead() then
...
local _calc_melee_hit_ray_original = PlayerStandard._calc_melee_hit_ray
function PlayerStandard:_calc_melee_hit_ray(...)
	local melee_entry = managers.blackmarket:equipped_melee_weapon()
	local melee_tweak = melee_entry and tweak_data.blackmarket.melee_weapons[melee_entry]
	local melee_stats = melee_tweak and melee_tweak.stats
	local original_range = melee_stats and melee_stats.range

	if original_range and managers.player:has_category_upgrade("player", "draw_of_the_sword_melee_range_multiplier") then
		melee_stats.range = original_range * managers.player:upgrade_value("player", "draw_of_the_sword_melee_range_multiplier", 1)
	end

	local result = _calc_melee_hit_ray_original(self, ...)

	if original_range then
		melee_stats.range = original_range
	end
...
local fray_check_action_primary_attack = PlayerStandard._check_action_primary_attack

function PlayerStandard:_check_action_primary_attack(t, input, params)
	params = params or {}

	if self._melee_stunned then
		params.action_forbidden = true
	end

	local was_shooting = self._shooting
	local new_action = fray_check_action_primary_attack(self, t, input, params)

	if not was_shooting and self._shooting and not self._shooting_t_pop then
...
function PlayerStandard:_start_action_unequip_weapon(t, data)
	...
	self:_interupt_action_reload(t)
	self:_interupt_action_steelsight(t)
	self._ext_network:send("switch_weapon", speed_multiplier, speed_multiplier)
end
...
function PlayerStandard:_start_action_equip_weapon(t)
	...
	if weapon_base._next_fire_allowed ~= nil then
...
local _check_action_throw_projectile_original = PlayerStandard._check_action_throw_projectile
function PlayerStandard:_check_action_throw_projectile(t, input)
	if self._melee_stunned then
		return
	end

	return _check_action_throw_projectile_original(self, t, input)
end
...
local _update_check_actions_original = PlayerStandard._update_check_actions
function PlayerStandard:_update_check_actions(t, dt, paused)
	self:_update_hh_action_timers(t, dt, true)

	return _update_check_actions_original(self, t, dt, paused)
end
