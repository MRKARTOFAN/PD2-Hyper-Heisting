...
function PlayerDamage:init(unit)
	...
	if FRAY.settings.toggle_health_effect == true then
...
function PlayerDamage:damage_melee(attack_data)
	...
	if go_through_armor or attack_data.is_cloaker_kick and not pm:has_category_upgrade("player", "hh_yakuza_base") then
...
function PlayerDamage:_send_damage_drama(attack_data, health_subtracted, armor)
	local max_damage = armor and self._max_armor and self:_max_armor() or self._max_health and self:_max_health() or self._HEALTH_INIT or 1
	local dmg_percent = health_subtracted / max_damage
...
function PlayerDamage:_calc_health_damage(attack_data)
	...
	local revive_reasons = self._jackpot_token
	...
	if health_subtracted > 0 and managers.player:has_category_upgrade("player", "yield_my_flesh_melee_damage_taken_multiplier") then
		managers.player:add_yield_my_flesh_health_damage(health_subtracted / self:_max_health())
	end
...
function PlayerDamage:build_suppression(amount)
	...
	if self._regenerate_timer then
		self._hh_sync_ehi_armor_regen = true
	end
end
...
function PlayerDamage:_update_regenerate_timer(t, dt)
	if self._hh_sync_ehi_armor_regen then
		self._hh_sync_ehi_armor_regen = nil
		self:_hh_update_ehi_armor_regen(t)
	end

	local dt_mul = self._regenerate_speed or 1
	dt_mul = dt_mul * math.lerp(1, managers.player:has_category_upgrade("player", "hh_armorer_suppression") and 0.625 or 0.25, self:suppression_ratio())
...
function PlayerDamage:_hh_update_ehi_armor_regen(t)
	local ehi = managers.ehi_buff
	if not ehi or not self._regenerate_timer then
		return
	end

	local work = self._regenerate_timer / (self._regenerate_speed or 1)
	local suppression = math.clamp(self:suppression_ratio(), 0, 1)
	local speed = 1 - suppression * 0.75
	local decay_delay = self._supperssion_data and self._supperssion_data.decay_start_t and math.max(0, self._supperssion_data.decay_start_t - t) or 0
	local delay_work = speed * decay_delay
	local remaining

	if work <= delay_work then
		remaining = work / speed
	else
		work = work - delay_work

		local decay_time = suppression * 10
		local decay_work = suppression * 10 - suppression * suppression * 3.75

		if work <= decay_work then
			remaining = decay_delay + (-speed + math.sqrt(speed * speed + 0.15 * work)) / 0.075
		else
			remaining = decay_delay + decay_time + work - decay_work
		end
	end

	ehi:AddBuff("ArmorRegenDelay", remaining)
end

Hooks:PostHook(PlayerDamage, "set_regenerate_timer_to_max", "hh_sync_ehi_armor_regen", function(self)
	self._hh_sync_ehi_armor_regen = true
end)
...
function PlayerDamage:update(unit, t, dt)
	...
	if not self._downed_timer and self._downed_progression then
		...
		if not _G.IS_VR then
			local blur = FRAY:BlurzoneEnabled() and self._downed_progression * 0.3 or self._downed_progression
	...
	if FRAY.need_to_reset_visuals then
		managers.environment_controller:set_chromatic_value_lerp(0)
		managers.environment_controller:set_contrast_value_lerp(0)
		FRAY.need_to_reset_visuals = nil
	end
...
function PlayerDamage:revive(silent)
	...
	managers.player:reset_yield_my_flesh_damage_ratio()
	...
	if managers.player:has_inactivate_temporary_upgrade("temporary", "surge_of_power_revive_melee_damage_multiplier") then
		managers.player:activate_temporary_upgrade("temporary", "surge_of_power_revive_melee_damage_multiplier")
	end
end
...
function PlayerDamage:_calc_armor_damage(...)
	...
	local health_subtracted = _calc_armor_damage_sh_original(self, ...)

	if health_subtracted > 0 and had_armor and self:get_real_armor() <= 0 and self._can_take_dmg_timer and self._can_take_dmg_timer <= 0 then
...
local _hh_max_health_original = PlayerDamage._max_health
function PlayerDamage:_max_health()
	if managers.player:has_category_upgrade("player", "hh_yakuza_base") then
		return 1 / (tweak_data.gui.stats_present_multiplier or 10)
	end

	local health = _hh_max_health_original(self)
	local bonus = 0

	if managers.player:has_category_upgrade("player", "hh_muscle_health_1") then
		bonus = bonus + 0.3
	end

	if managers.player:has_category_upgrade("player", "hh_muscle_health_2") then
		bonus = bonus + 0.3
	end
...
local _hh_set_regenerate_timer_to_max_original = PlayerDamage.set_regenerate_timer_to_max
function PlayerDamage:set_regenerate_timer_to_max()
	_hh_set_regenerate_timer_to_max_original(self)

	if managers.player:has_category_upgrade("player", "hh_armorer_regen") then
		self._regenerate_timer = self._regenerate_timer / 1.15
	end

	if managers.player:has_category_upgrade("player", "hh_yakuza_regen") then
		self._regenerate_timer = self._regenerate_timer / (self:armor_ratio() < 0.5 and 1.2 or 1.1)
	end
end

function PlayerDamage:hh_grinder_stack_cap()
...
function PlayerDamage:_calc_armor_damage(attack_data)
	if managers.player:has_category_upgrade("player", "hh_yakuza_base") then
		...
		attack_data.melee_armor_piercing = nil
	end

	local had_armor = self:get_real_armor() > 0
	local health_subtracted = _hh_calc_armor_damage_original(self, attack_data)

	if health_subtracted > 0 then
		managers.player:hh_maniac_damage()
	end

	if had_armor and self:get_real_armor() <= 0 and managers.player:has_enabled_cooldown_upgrade("cooldown", "armor_break_invulnerable") then
		self._can_take_dmg_timer = tweak_data.upgrades.values.player.armor_break_invulnerable_duration[1] or 0
		managers.player:disable_cooldown_upgrade("cooldown", "armor_break_invulnerable")
		attack_data.armor_piercing = nil
	end
...
local _hh_calc_health_damage_original = PlayerDamage._calc_health_damage
function PlayerDamage:_calc_health_damage(attack_data)
	if managers.player:has_category_upgrade("player", "hh_muscle_health_resistance") then
		attack_data.damage = attack_data.damage * 0.85
	end

	local health_subtracted = _hh_calc_health_damage_original(self, attack_data)
	if health_subtracted > 0 then
		managers.player:hh_maniac_damage()
	end
...
local _hh_chk_cheat_death_original = PlayerDamage._chk_cheat_death
function PlayerDamage:_chk_cheat_death(...)
	if managers.player:has_category_upgrade("player", "hh_yakuza_cheat_death") and self:get_real_health() <= 0 then
		local t = Application:time()
		if not self._hh_yakuza_cheat_death_t or self._hh_yakuza_cheat_death_t <= t then
			self:set_health(self:_max_health())
			self:set_armor(self:_max_armor())
			self:_send_set_health()
			self:_send_set_armor()
			self._unit:sound():play("pickup_fak_skill")
			self._hh_yakuza_cheat_death_t = t + 360
			return true
		end
	end

	return _hh_chk_cheat_death_original(self, ...)
end
