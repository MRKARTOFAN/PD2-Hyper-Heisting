...
function CopLogicTravel._get_allowed_travel_nav_segs(data, ...)
	-- Returning nothing allows all nav segments (used for continuous follow path)
	if CopLogicTravel._use_segmented_coarse_path(data) then
...
Hooks:PreHook(CopLogicTravel, "upd_advance", "hh_upd_advance", function(data)
	local my_data = data.internal_data
	local t = TimerManager:game():time()
	if my_data.cover_leave_t and my_data.cover_leave_t < t and not data.unit:movement():chk_action_forbidden("walk") and not data.unit:anim_data().reload then
		my_data.cover_leave_t = nil
	end

	CopLogicTravel._chk_relocate_follow(data, my_data)
end)

function CopLogicTravel._chk_relocate_follow(data, my_data, max_dis)
	local objective = data.objective
	...
	if not destination_pos or mvec3_dis_sq(destination_pos, follow_pos) < (max_dis or 3000) ^ 2 then
		return
	end
...
local _chk_group_ready_to_move_original = CopLogicTravel.chk_group_ready_to_move
...
-- Helper: find a position behind a unit to use as cover
function CopLogicTravel._get_pos_behind_unit(data, unit, min_dis, max_dis)
	local threat_dir = tmp_vec1
	local threat_side = tmp_vec2
	local pos = tmp_vec3
	local unit_pos = tmp_vec4
	...
	if walk_to_pos then
		mvec3_lerp(unit_pos, unit_pos, walk_to_pos, math.min(mvec3_dis(unit_pos, data.m_pos) / 1000, 1))
	end
...
Hooks:PostHook(CopLogicTravel, "action_complete_clbk", "hh_action_complete_clbk", function(data, action)
...
-- Allows enemies to start their new path immediately
Hooks:PreHook(CopLogicTravel, "exit", "hh_travel_exit", function(data, new_logic_name)
...
-- Fix enemies sometimes disappearing when told to retire
...
Hooks:PostHook(CopLogicTravel, "queued_update", "hh_travel_queued_update", function(data)
