...
Hooks:PostHook(CoreEnvironmentControllerManager, "init", "fraypost_env", function(self)
...
function CoreEnvironmentControllerManager:_update_dof(t, dt)
	...
	if self._dof_override then
		...
		local no_weird_dof = FRAY.settings.toggle_noweirddof == true
...
Hooks:OverrideFunction(CoreEnvironmentControllerManager, "test_line_of_sight", function(self, test_pos, min_distance, dot_distance, max_distance, is_flashbang)
...
if FRAY and FRAY:BlurzoneEnabled() then
