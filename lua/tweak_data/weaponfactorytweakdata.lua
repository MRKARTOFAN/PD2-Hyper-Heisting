...
for part_id, part in pairs(self.parts) do
	if part.type == "sight" then
		sight_parts[part_id] = true

		if part.stats then
			part.stats.concealment = nil
		end
	end
end

for _, factory in pairs(self) do
	if type(factory) == "table" and factory.override then
		for part_id, override in pairs(factory.override) do
			if sight_parts[part_id] and type(override) == "table" and override.stats then
				override.stats.concealment = nil
			end
		end
	end
end
