...
local FRAY_WEAPON_TOTAL_DAMAGE = 720
local FRAY_WEAPON_PICKUP_DAMAGE = 32
local FRAY_WEAPON_DAMAGE_TIERS = {
	[24] = { "shuno" },
	[32] = { "mp9", "scorpion", "tec9", "baka", "pm9", "fmg9", "glock_18c", "czech", "beer", "hailstorm" },
	[36] = { "new_mp5", "coal", "shepheard", "stech", "m249", "mg42", "tecci", "kacchainsaw", "m134" },
	[40] = { "amcar", "g36", "olympic", "s552", "famas", "corgi", "mp7", "p90", "m1928", "cobray", "sr2", "glock_17", "b92fs", "g26", "pl14", "packrat", "holt", "c96", "car9" },
	[48] = { "new_m4", "ak5", "ak74", "aug", "vhs", "hajk", "komodo", "tkb", "mac10", "uzi", "polymer", "vityaz", "legacy", "maxim9", "rpk", "hk51b", "sg416", "ak5s", "smolak" },
	[60] = { "m16", "l85a2", "groza", "m45", "erma", "sterling", "schakal", "p226", "g22c", "hs2000", "sparrow", "hk21", "par", "m60", "spike" },
	[72] = { "akm", "akm_gold", "akmsu", "flint", "speen", "usp", "ppk", "pmm", "lemming", "shrew" },
	[80] = { "sub2000", "colt_1911", "m1911", "type54", "cold" },
	[96] = { "scar", "g3", "galil", "fal", "asval", "breech" },
	[128] = { "new_m14", "contraband", "shak12", "hcar", "mateba", "korth" },
	[144] = { "ching" },
	[160] = { "deagle", "new_raging_bull", "peacemaker", "chinchilla", "model3" },
	[192] = { "welrod", "rsh12" }
}
local FRAY_DMR_WEAPONS = table.list_to_set({ "sub2000", "scar", "g3", "galil", "fal", "asval", "new_m14", "contraband", "shak12", "hcar", "ching" })
local FRAY_WEAPON_CATEGORIES = table.list_to_set({ "assault_rifle", "smg", "pistol", "revolver", "lmg", "minigun" })
local FRAY_EXCLUDED_WEAPON_CATEGORIES = table.list_to_set({ "shotgun", "snp", "grenade_launcher", "bow", "crossbow", "flamethrower", "saw" })

local function FRAYGetWeaponCategories(weapon)
	return table.list_to_set(weapon.categories or {})
end

local function FRAYCanRebalanceWeapon(weapon)
	if type(weapon) ~= "table" or not weapon.stats or not weapon.categories then
		return false
	end

	local selection_index = weapon.use_data and weapon.use_data.selection_index

	if selection_index ~= 1 and selection_index ~= 2 then
		return false
	end

	local categories = FRAYGetWeaponCategories(weapon)
	local is_conventional = false

	for category in pairs(FRAY_WEAPON_CATEGORIES) do
	...
	if not is_conventional then
		return false
	end

	for category in pairs(FRAY_EXCLUDED_WEAPON_CATEGORIES) do
		if categories[category] then
			return false
		end
	end
...
local function FRAYSetWeaponDamage(weapon, damage)
	weapon.stats.damage = damage

	if weapon.stats_modifiers then
		weapon.stats_modifiers.damage = nil

		if not next(weapon.stats_modifiers) then
			weapon.stats_modifiers = nil
		end
	end
end

local function FRAYRebalanceWeaponAmmo(self, weapon_id, weapon, is_akimbo, based_on_id)
	local categories = FRAYGetWeaponCategories(weapon)
	local total_ammo_mul = 1
	local pickup_mul = 1

	if categories.minigun then
		total_ammo_mul = 2.25
	elseif categories.lmg then
		local concealment = weapon.stats.concealment or 15

		total_ammo_mul = math.round(math.map_range_clamped(concealment, 15, 20, 9 / 5, 7 / 5), 0.05)
		pickup_mul = 1.7
	elseif categories.smg then
		total_ammo_mul = 90 / 80
	elseif categories.revolver then
		pickup_mul = 71 / 100
	elseif categories.pistol then
		total_ammo_mul = weapon.auto and 1 or 1.5
		pickup_mul = 40 / 30
	end

	local is_dmr = FRAY_DMR_WEAPONS[weapon_id] or FRAY_DMR_WEAPONS[based_on_id] or categories.dmr

	if is_dmr then
		pickup_mul = pickup_mul * 30 / 50
	end

	if is_akimbo or categories.akimbo then
		total_ammo_mul = 1.25
	end

	if weapon.use_data.selection_index == 1 then
		total_ammo_mul = total_ammo_mul * 0.5
		pickup_mul = pickup_mul * 0.75
	end

	if weapon.can_shoot_through_wall then
		total_ammo_mul = total_ammo_mul * 0.5
		pickup_mul = pickup_mul * 0.25
		weapon.ammo_bag_consumption_mul = (weapon.ammo_bag_consumption_mul or 1) * 1.25
	end

	local damage_modifier = weapon.stats_modifiers and weapon.stats_modifiers.damage or 1
	local damage_stat = math.min(weapon.stats.damage, #self.stats.damage)
	local real_damage = self.stats.damage[damage_stat] * damage_modifier
	local clip_damage = weapon.CLIP_AMMO_MAX and weapon.CLIP_AMMO_MAX * real_damage

	if weapon.AMMO_MAX and clip_damage and clip_damage > 0 then
		weapon.NR_CLIPS_MAX = math.max(1, math.round(FRAY_WEAPON_TOTAL_DAMAGE * total_ammo_mul / clip_damage))
		weapon.NR_CLIPS_MAX = math.round(weapon.NR_CLIPS_MAX, weapon.max_clips_round or 1)
		weapon.NR_CLIPS_MAX = math.max(weapon.NR_CLIPS_MAX, weapon.min_max_clips or 0)
		weapon.AMMO_MAX = weapon.CLIP_AMMO_MAX * weapon.NR_CLIPS_MAX
	end

	if weapon.AMMO_PICKUP and weapon.AMMO_PICKUP[2] > 0 then
		local pickup_damage_max = FRAY_WEAPON_PICKUP_DAMAGE * pickup_mul
		local pickup_damage_min = pickup_damage_max / 2

		weapon.AMMO_PICKUP = {
			math.round(math.floor(pickup_damage_min / real_damage * 100) / 100, 0.05),
			math.round(math.floor(pickup_damage_max / real_damage * 100) / 100, 0.05)
		}
	end
end

local function FRAYRebalanceWeapons(self)
	local weapon_damage = {}
	local based_on = {}

	for damage, weapon_ids in pairs(FRAY_WEAPON_DAMAGE_TIERS) do
		for _, weapon_id in ipairs(weapon_ids) do
			if FRAYCanRebalanceWeapon(self[weapon_id]) then
				weapon_damage[weapon_id] = damage
			end
		end
	end

	local added_custom_weapon = true

	while added_custom_weapon do
		added_custom_weapon = false

		for weapon_id, weapon in pairs(self) do
			local based_on_id = type(weapon) == "table" and weapon.custom and weapon.based_on

			if not weapon_damage[weapon_id] and based_on_id and weapon_damage[based_on_id] and FRAYCanRebalanceWeapon(weapon) then
				weapon_damage[weapon_id] = weapon_damage[based_on_id]
				based_on[weapon_id] = based_on_id
				added_custom_weapon = true
			end
		end
	end

	local akimbo_weapons = {}
	local akimbo_mappings = self:get_akimbo_mappings() or {}

	for weapon_id, damage in pairs(weapon_damage) do
		local akimbo_id = akimbo_mappings[weapon_id] or "x_" .. weapon_id

		if FRAYCanRebalanceWeapon(self[akimbo_id]) then
			akimbo_weapons[akimbo_id] = weapon_id
		end
	end

	for akimbo_id, single_id in pairs(akimbo_weapons) do
		weapon_damage[akimbo_id] = weapon_damage[single_id]
		based_on[akimbo_id] = single_id
	end

	for weapon_id, damage in pairs(weapon_damage) do
		FRAYSetWeaponDamage(self[weapon_id], damage)
	end

	for weapon_id in pairs(weapon_damage) do
		FRAYRebalanceWeaponAmmo(self, weapon_id, self[weapon_id], akimbo_weapons[weapon_id] ~= nil, based_on[weapon_id])
	end
end
...
local shotgun_tuning = {
	rota = {pellet_damage = 22, near = 600, far = 1400},
	x_rota = {pellet_damage = 22, near = 600, far = 1400},
	r870 = {pellet_damage = 28, near = 800, far = 1700},
	serbu = {pellet_damage = 28, near = 700, far = 1500},
	m1897 = {pellet_damage = 28, near = 850, far = 1800},
	benelli = {pellet_damage = 22, near = 600, far = 1400},
	spas12 = {pellet_damage = 22, near = 750, far = 1600},
	aa12 = {pellet_damage = 16, near = 650, far = 1400},
	ksg = {pellet_damage = 28, near = 900, far = 1900},
	ultima = {pellet_damage = 22, near = 650, far = 1400},
		basset = {pellet_damage = 16, near = 550, far = 1250},
		x_basset = {pellet_damage = 16, near = 550, far = 1250},
	saiga = {pellet_damage = 18, near = 700, far = 1500},
	coach = {pellet_damage = 36, near = 1000, far = 2100},
	huntsman = {pellet_damage = 36, near = 1100, far = 2300},
	b682 = {pellet_damage = 36, near = 1200, far = 2400},
	judge = {pellet_damage = 28, near = 650, far = 1400},
	x_judge = {pellet_damage = 28, near = 650, far = 1400},
	boot = {pellet_damage = 32, near = 900, far = 1900},
	striker = {pellet_damage = 20, near = 650, far = 1450},
	m37 = {pellet_damage = 32, near = 800, far = 1700},
	sko12 = {pellet_damage = 16, near = 650, far = 1400},
	x_sko12 = {pellet_damage = 16, near = 550, far = 1250},
	supernova = {pellet_damage = 28, near = 850, far = 1800, alt_damage_mul = 22 / 28},
	m590 = {pellet_damage = 28, near = 800, far = 1700},
	slap = {pellet_damage = 36, near = 1000, far = 2100},
...
for weapon_id, data in pairs(shotgun_tuning) do
	local weapon = self[weapon_id]

	if weapon then
		if weapon.rays ~= 16 then
			weapon.rays = 8
		end

		local damage = data.pellet_damage * weapon.rays
		local pickup_max = 40 / data.pellet_damage

		for damage_index = #self.stats.damage + 1, damage do
			self.stats.damage[damage_index] = damage_index / 10
		end

		weapon.stats.damage = damage
		weapon.damage_near = data.near
		weapon.damage_far = data.far

		if data.alt_damage_mul and weapon.alt_fire_data then
			weapon.alt_fire_data.damage_mul = data.alt_damage_mul
		end

		weapon.AMMO_PICKUP = {
			math.max(math.round(pickup_max / 2 - 0.2, 0.05), 0.05),
			math.max(math.round(pickup_max - 0.3, 0.05), 0.05)
		}
...
FRAYRebalanceWeapons(self)

for _, weapon_id in ipairs({ "hk21", "par", "m60" }) do
	local weapon = self[weapon_id]
	weapon.NR_CLIPS_MAX = weapon.NR_CLIPS_MAX + 1
	weapon.AMMO_MAX = weapon.CLIP_AMMO_MAX * weapon.NR_CLIPS_MAX
end
