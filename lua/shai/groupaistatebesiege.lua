...
local hh_minigun_dozer_units = {
	[Idstring("units/pd2_dlc_drm/characters/ene_bulldozer_minigun/ene_bulldozer_minigun"):key()] = true,
	[Idstring("units/pd2_dlc_drm/characters/ene_bulldozer_minigun_classic/ene_bulldozer_minigun_classic"):key()] = true,
	[Idstring("units/pd2_dlc_mad/characters/ene_akan_dozer_mini/ene_akan_dozer_mini"):key()] = true,
	[Idstring("units/pd2_mod_psc/characters/ene_murkywater_bulldozer_1/ene_murkywater_bulldozer_1"):key()] = true,
	[Idstring("units/pd2_dlc_bph/characters/ene_murkywater_bulldozer_1/ene_murkywater_bulldozer_1"):key()] = true,
	[Idstring("units/pd2_dlc_bex/characters/ene_swat_dozer_policia_federale_minigun/ene_swat_dozer_policia_federale_minigun"):key()] = true
}

local function FRAYIsMinigunDozer(unit_name)
	return hh_minigun_dozer_units[unit_name:key()]
end

local function FRAYHasActiveMinigunDozer(state)
	for _, unit_data in pairs(state._police) do
		local unit = unit_data.unit

		if alive(unit) and unit:base()._tweak_table == "tank_mini" then
			return true
		end
	end
end

local function FRAYPickSpawnUnit(state, units)
	local block_minigun = state._hh_next_minigun_dozer_t and state._t < state._hh_next_minigun_dozer_t or FRAYHasActiveMinigunDozer(state)
	local start_index = math.random(#units)

	for offset = 0, #units - 1 do
		local unit_name = units[(start_index + offset - 1) % #units + 1]

		unit_name = managers.modifiers:modify_value("GroupAIStateBesiege:SpawningUnit", unit_name)

		if not block_minigun or not FRAYIsMinigunDozer(unit_name) then
			return unit_name
		end
	end
end
...
Hooks:PostHook(GroupAIStateBesiege, "init", "fray_level_ai_flags", function(self)
...
Hooks:PostHook(GroupAIStateBesiege, "_set_objective_to_enemy_group", "fray_street_objectives", function(self, group, objective)
	if self._street and objective and objective.type == "assault_area" then
		objective.attitude = "avoid"
	end
end)
-- [Karto] Incomplete port of flags (mainly street here). For "street", makes the enemies less pushy and more "avoid", so to speak. "meatgrinder" just reduces pool and force values. Gotta play more maps to make conclusion.
...
function GroupAIStateBesiege:_begin_assault_task(...)
	...
	local reenforce_task = self._task_data.reenforce
	local reenforce_delay = tweak_data.group_ai.init_reenforce_delay or 30
	reenforce_task.next_dispatch_t = math.max(reenforce_task.next_dispatch_t or 0, self._t + reenforce_delay)
end
...
-- Track multiple reinforcement IDs in the same area so one removed point does not clear or overwrite another.
Hooks:OverrideFunction(GroupAIStateBesiege, "set_area_min_police_force", function(self, id, force, pos)
	...
	if force then
		...
		if force_factor.id and force_factor.force and not force_factor.ids[force_factor.id] then
			force_factor.ids[force_factor.id] = force_factor.force
		end
		...
		for _, area in pairs(self._area_data) do
			...
			if force_factor and force_factor.ids and force_factor.ids[id] then
				...
				return
			elseif force_factor and force_factor.id == id then
				area.factors.force = nil
				return
			end
		end
...
function GroupAIStateBesiege:_hh_get_nearby_criminal_area(area)
	if not area then
		return
	end
	...
	for _, neighbour_area in pairs(area.neighbours) do
...
Hooks:OverrideFunction(GroupAIStateBesiege, "_set_assault_objective_to_group", function(self, group, phase)
	...
	local pressure_area = not phase_is_anticipation and self:_hh_get_nearby_criminal_area(objective_area)
	...
	if tactics_map.deathguard and not phase_is_anticipation then
		if current_objective.tactic == "deathguard" then
			...
			local nav_seg = u_data and u_data.tracker and u_data.tracker:nav_segment()
			if u_data and u_data.status and current_objective.area and current_objective.area.nav_segs[nav_seg] then
				return
			else
		...
		for _, u_data in pairs(self._char_criminals) do
			if alive(u_data.unit) and u_data.status and u_data.status ~= "dead" then
	...
	if current_objective.open_fire then
		if not current_objective.moving_out and pressure_area and not tactics_map.ranged_fire then
			local open_fire_t = current_objective.assigned_t or group.in_place_t or self._t
			local push_delay = tactics_map.charge and 2 or tactics_map.no_push and 5 or 2
			approach = self._t - open_fire_t > push_delay or in_place_duration > push_delay
		end

		if not approach and not current_objective.moving_out and (tactics_map.charge or not tactics_map.ranged_fire or in_place_duration > 10) then
		...
		if pressure_area and not tactics_map.ranged_fire and (tactics_map.charge or tactics_map.flank) then
			open_fire = true
		elseif phase_is_anticipation or obstructed_area or tactics_map.ranged_fire then
		...
		if assault_area and assault_path then
			...
			if push then
				...
				if phase_is_anticipation then
					...
					if self._hostage_headcount > 0 and not group.said_standby and time_until_phase_end > 3 and in_place_duration > 1 then
				...
				if not self:_chk_group_use_grenade(assault_area, group, detonate_pos) then
					if not group.ignore_grenade_check_t then
						local no_grenade_push_delay = tweak_data.group_ai.no_grenade_push_delay or 5
						local delay = no_grenade_push_delay * (tactics_map.charge and 0.25 or 1)
...
function GroupAIStateBesiege:_chk_group_use_grenade(assault_area, group, detonate_pos)
	...
	if use_teargas then
		...
		timeout = tweak_data.group_ai.cs_grenade_timeout or tweak_data.group_ai.smoke_and_flash_grenade_timeout or { 30, 20 }
	else
		...
		timeout = tweak_data.group_ai[grenade_type .. "_timeout"] or tweak_data.group_ai.smoke_and_flash_grenade_timeout or { 10, 20 }
	end

	task_data.use_smoke = false
	-- Minimum grenade cooldown
	local min_grenade_timeout = tweak_data.group_ai.min_grenade_timeout or 30
...
function GroupAIStateBesiege:_perform_group_spawning(spawn_task, force)
		...
		for _, sp_data in ipairs(spawn_task.spawn_group.spawn_pts) do
			...
			if has_access and (not sp_data.amount or sp_data.amount > 0) and sp_data.mission_element:enabled() then
				...
				if (skip_delays and sp_data.delay_t - sp_data.interval or sp_data.delay_t) < self._t then
					...
					if not PackageManager:has(IDS_UNIT, produce_data.name) then
						managers.dyn_resource:load(IDS_UNIT, produce_data.name, managers.dyn_resource.DYN_RESOURCES_PACKAGE)
					end

					local spawned_unit = sp_data.mission_element:produce(produce_data)

					if FRAYIsMinigunDozer(produce_data.name) then
						self._hh_next_minigun_dozer_t = self._t + (Global.game_settings and Global.game_settings.one_down and 90 or 150)
					end
	...
	for u_type_name, spawn_info in pairs(spawn_task.units_remaining) do
		local u_category = unit_categories[u_type_name]
		if u_category and not u_category.access.acrobatic then
	...
	local spawn_cooldown_mul = tweak_data.group_ai.spawn_cooldown_mul or 1
	self:_set_objective_type_cooldown(spawn_task.group.objective.type, spawn_task.group.size * spawn_cooldown_mul)
end
...
Hooks:OverrideFunction(GroupAIStateBesiege, "_set_reenforce_objective_to_group", function(self, group)
	...
	elseif next(target_area.criminal.units) then
		...
		elseif not self:_chk_group_use_grenade(target_area, group) then
			local no_grenade_push_delay = tweak_data.group_ai.no_grenade_push_delay or 5
			if in_place_duration < no_grenade_push_delay * 0.5 then
				move_in = false
			end
		end
		if not move_in then
...
Hooks:OverrideFunction(GroupAIStateBesiege, "_set_recon_objective_to_group", function(self, group)
	...
	self:_set_objective_to_enemy_group(group, {
		...
		stance = "hos",
		pose = move_in and "crouch" or "stand",
		attitude = move_in and next(target_area.criminal.units) and "engage" or "avoid",
		moving_in = move_in,
		area = self:get_area_from_nav_seg_id(coarse_path[#coarse_path][1]),
		target_area = target_area,
		coarse_path = coarse_path
...
local hh_fray_choose_best_group_original = GroupAIStateBesiege._choose_best_group
function GroupAIStateBesiege:_choose_best_group(...)
	local spawn_group, spawn_group_type = hh_fray_choose_best_group_original(self, ...)

	if spawn_group and Global.game_settings and Global.game_settings.one_down then
		local remaining_delay = math.max(0, spawn_group.delay_t - self._t)
		spawn_group.delay_t = self._t + remaining_delay * 0.25
	end
