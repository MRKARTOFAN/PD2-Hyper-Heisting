...
Hooks:PostHook(ElementSpecialObjective, "on_executed", "fray_delay_interrupt_so", function(self)
	if not self:editor_name():find("interrupt", 1, true) then
		return
	end

	local state = managers.groupai:state()
	local so = state._special_objectives[self._id]
	if so then
		-- [Karto] SO re-registrating after each fix. blehhhh
		so.chance = 0.25
		so.chance_inc = 0
		so.interval = 30
		so.delay_t = (state._t or TimerManager:game():time()) + 30
	end
end)
