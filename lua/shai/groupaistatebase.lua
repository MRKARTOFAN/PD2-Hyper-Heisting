...
Hooks:PostHook(GroupAIStateBase, "on_enemy_unregistered", "sh_on_enemy_unregistered", function(self, unit)
	...
	local max_dis = tweak_data.group_ai.spawn_kill_max_dis or 1500
	if dis > max_dis then
		return
	end

	local spawn_kill_cooldown = tweak_data.group_ai.spawn_kill_cooldown or 30
	local delay_t = self._t + math.map_range(dis, 0, max_dis, spawn_kill_cooldown, 0)
