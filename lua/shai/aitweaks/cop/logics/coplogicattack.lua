...
local hh_fray_chk_wants_to_take_cover_original = CopLogicAttack._chk_wants_to_take_cover
function CopLogicAttack._chk_wants_to_take_cover(data, my_data, ...)
	local group_objective = data.group and data.group.objective
	local drama_data = managers.groupai:state()._drama_data
	local high_drama = drama_data and drama_data.hh_zone == "high"
	local is_special = data.unit:base():has_tag("special")

	if high_drama and not is_special and group_objective and group_objective.type == "assault_area" and my_data.attitude == "engage" and not my_data.moving_to_cover and not data.is_suppressed then
		return
	end

	if not Global.game_settings or not Global.game_settings.one_down or not data.is_suppressed then
		return hh_fray_chk_wants_to_take_cover_original(data, my_data, ...)
	end

	local is_suppressed = data.is_suppressed
	data.is_suppressed = nil
	local result = hh_fray_chk_wants_to_take_cover_original(data, my_data, ...)
	data.is_suppressed = is_suppressed
