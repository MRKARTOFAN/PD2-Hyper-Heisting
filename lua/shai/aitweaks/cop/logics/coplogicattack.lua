...
function CopLogicAttack._chk_say_chatter(data, chatter_type, cooldown)
	if data.unit:base():has_tag("special") then
		if not data.unit:sound():speaking(data.t) then
			local chatter_data = tweak_data.group_ai.enemy_chatter and tweak_data.group_ai.enemy_chatter[chatter_type]
			if chatter_data and chatter_data.queue then
				data.unit:sound():say(chatter_data.queue, true)
