...
local _chk_start_melee_original = CopActionShoot._chk_start_melee
function CopActionShoot:_chk_start_melee(t, target_dis, target_pos)
	if self._unit:base():has_tag("shield") then
		return
	end

	return _chk_start_melee_original(self, t, target_dis, target_pos)
end

-- [Karto] HH drama stuff
local function hh_fray_enabled()
	return Global.game_settings and Global.game_settings.one_down
end

local hh_fray_update_original = CopActionShoot.update
function CopActionShoot:update(...)
	if not hh_fray_enabled() or not self._common_data.is_suppressed then
		return hh_fray_update_original(self, ...)
	end

	local is_suppressed = self._common_data.is_suppressed
	self._common_data.is_suppressed = nil
	local result = hh_fray_update_original(self, ...)
	self._common_data.is_suppressed = is_suppressed

	return result
end

local hh_fray_get_unit_shoot_pos_original = CopActionShoot._get_unit_shoot_pos
function CopActionShoot:_get_unit_shoot_pos(...)
	if not hh_fray_enabled() then
		return hh_fray_get_unit_shoot_pos_original(self, ...)
	end

	local common_data = self._common_data
	local is_suppressed = common_data.is_suppressed
	local lower_body_action = common_data.active_actions[2]
	local is_dodging = lower_body_action and lower_body_action:type() == "dodge"

	common_data.is_suppressed = nil
	if is_dodging then
		common_data.active_actions[2] = nil
	end

	local result = hh_fray_get_unit_shoot_pos_original(self, ...)
	common_data.is_suppressed = is_suppressed
	if is_dodging then
		common_data.active_actions[2] = lower_body_action
	end
